...
class PlaybackController extends Notifier<PlaybackState> {
  ...
  PlaybackItem _buildQueueItemFromTrack(Track track) {
    ...
    if (localItem != null && localItem.filePath.isNotEmpty) {
      if (isCueVirtualPath(localItem.filePath)) {
        return PlaybackItem(
          id: localItem.id,
    ...
    if (historyItem != null && historyItem.filePath.isNotEmpty) {
      if (isCueVirtualPath(historyItem.filePath)) {
  ...
  void playNext(Track track) {
    ...
    final insertIndex = (state.currentIndex + 1)
        ...
        .toInt();
    final newQueue = [...state.queue]..insert(insertIndex, item);
    state = state.copyWith(queue: newQueue);
    unawaited(_savePlaybackSnapshot());
    if (state.shuffle) _regenerateShuffleOrderPreservingCurrentProgress();
  }
  ...
  void reorderQueue(int oldIndex, int newIndex) {
    ...
    state = state.copyWith(queue: newQueue, currentIndex: nextCurrentIndex);
    unawaited(_savePlaybackSnapshot());
    if (state.shuffle) _regenerateShuffleOrderPreservingCurrentProgress();
  }
  ...
  void clearUpcoming() {
    ...
    final retained = state.queue.sublist(0, state.currentIndex + 1);
    state = state.copyWith(queue: retained);
    unawaited(_savePlaybackSnapshot());
    if (state.shuffle) _regenerateShuffleOrderPreservingCurrentProgress();
  }
  ...
  Future<List<Track>> _searchTracksForSmartQueue(
    ...
    final settings = ref.read(settingsProvider);
    final preferSpotify =
        (settings.searchProvider ?? '').trim().toLowerCase() == 'spotify';
